...
constexpr auto kBgGhostDeletedPrefKey = "bettergram-ghost-deleted";
...
[[nodiscard]] QByteArray SerializeBgGhostDeleted(
		const base::flat_map<FullMsgId, Data::BgGhostContent> &entries) {
	if (entries.empty()) {
		return {};
	}
	auto stream = Serialize::ByteArrayWriter(
		sizeof(quint32) + sizeof(qint64) * 3 * entries.size());
	stream << quint32(entries.size());
	for (const auto &[id, content] : entries) {
		stream
			<< qint64(id.peer.value)
			<< qint64(id.msg.bare)
			<< content.senderName
			<< content.text
			<< qint64(content.date);
...
[[nodiscard]] base::flat_map<FullMsgId, Data::BgGhostContent>
DeserializeBgGhostDeleted(const QByteArray &serialized) {
	auto result = base::flat_map<FullMsgId, Data::BgGhostContent>();
	...
	for (auto i = quint32(0); i != count; ++i) {
		auto peer = qint64();
		auto msg = qint64();
		auto content = Data::BgGhostContent();
		auto date = qint64();
		stream >> peer >> msg >> content.senderName >> content.text >> date;
		if (!stream.ok()) {
			return {};
		}
		content.date = TimeId(date);
		result.emplace(FullMsgId(PeerId(peer), MsgId(msg)), content);
	}
	return stream.atEnd()
		? result
		: base::flat_map<FullMsgId, Data::BgGhostContent>();
}
...
, _shortcutMessages(std::make_unique<ShortcutMessages>(this)) {
	...
	_bgGhostDeleted = DeserializeBgGhostDeleted(
		_session->local().readPrefBytes(kBgGhostDeletedPrefKey).value_or(
			QByteArray()));
...
void Session::applyBgGhostDeleted(not_null<HistoryItem*> item) {
	if (_bgGhostDeleted.contains(item->fullId())) {
		item->setBgGhostDeleted();
	}
}

void Session::saveBgGhostDeleted() {
	if (_bgGhostDeleted.empty()) {
		_session->local().clearPref(kBgGhostDeletedPrefKey);
		return;
	}
	_session->local().writePrefBytes(
		kBgGhostDeletedPrefKey,
		SerializeBgGhostDeleted(_bgGhostDeleted));
}

bool Session::shouldKeepAsBgGhost(not_null<const HistoryItem*> item) const {
	return Core::App().settings().bgGhostMessages()
		&& !item->out()
		&& !item->isService();
}

Data::BgGhostContent Session::CaptureBgGhostContent(
		not_null<const HistoryItem*> item) const {
	return Data::BgGhostContent{
...
auto Session::bgGhostVault() const
-> const base::flat_map<FullMsgId, Data::BgGhostContent> & {
	return _bgGhostDeleted;
}

void Session::removeBgGhostVaultEntry(FullMsgId id) {
	_bgGhostDeleted.remove(id);
	saveBgGhostDeleted();
...
void Session::processMessagesDeleted(
	...
	if (!toDestroy.empty()) {
		...
		const auto ghostMode = Core::App().settings().bgGhostMessages();
		for (const auto &item : toDestroy) {
			if (ghostMode && shouldKeepAsBgGhost(item)) {
				item->setBgGhostDeleted();
				_bgGhostDeleted.emplace(
					item->fullId(),
					CaptureBgGhostContent(item));
