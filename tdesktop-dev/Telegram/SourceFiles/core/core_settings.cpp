...
std::optional<QByteArray> Settings::readPrefGeneric(std::string_view key) const {
	const auto i = _prefs.find(QByteArray(key.data(), key.size()));
...
void Settings::writePrefImpl<bool>(std::string_view key, bool value) {
	writePrefGeneric(key, value ? QByteArray(1, '\x01') : QByteArray());
}
...
void Settings::setTranslateUrlTemplate(const QString &value) {
	if (value.isEmpty()) {
		clearPref("translate-url-template");
	} else {
		writePrefGeneric("translate-url-template", value.toUtf8());
	}
	_bettergramChanged.fire({});
}

QString Settings::translateUrlTemplate() const {
	if (const auto data = readPrefGeneric("translate-url-template")) {
		return QString::fromUtf8(*data);
	}
	return {};
}

void Settings::setWhisperApiUrl(const QString &value) {
	if (value.isEmpty()) {
		clearPref("whisper-api-url");
	} else {
		writePrefGeneric("whisper-api-url", value.toUtf8());
	}
	_bettergramChanged.fire({});
}

QString Settings::whisperApiUrl() const {
	if (const auto data = readPrefGeneric("whisper-api-url")) {
		return QString::fromUtf8(*data);
	}
	return {};
}

void Settings::setWhisperApiKey(const QString &value) {
	if (value.isEmpty()) {
		clearPref("whisper-api-key");
	} else {
		writePrefGeneric("whisper-api-key", value.toUtf8());
	}
	_bettergramChanged.fire({});
}

QString Settings::whisperApiKey() const {
	if (const auto data = readPrefGeneric("whisper-api-key")) {
		return QString::fromUtf8(*data);
	}
	return {};
}

void Settings::setAiComposeApiUrl(const QString &value) {
	if (value.isEmpty()) {
		clearPref("ai-compose-api-url");
	} else {
		writePrefGeneric("ai-compose-api-url", value.toUtf8());
	}
	_bettergramChanged.fire({});
}

QString Settings::aiComposeApiUrl() const {
	if (const auto data = readPrefGeneric("ai-compose-api-url")) {
		return QString::fromUtf8(*data);
	}
	return {};
}

void Settings::setAiComposeApiKey(const QString &value) {
	if (value.isEmpty()) {
		clearPref("ai-compose-api-key");
	} else {
		writePrefGeneric("ai-compose-api-key", value.toUtf8());
	}
	_bettergramChanged.fire({});
}

QString Settings::aiComposeApiKey() const {
	if (const auto data = readPrefGeneric("ai-compose-api-key")) {
		return QString::fromUtf8(*data);
	}
	return {};
}

void Settings::setAiComposeApiModel(const QString &value) {
	if (value.isEmpty()) {
		clearPref("ai-compose-api-model");
	} else {
		writePrefGeneric("ai-compose-api-model", value.toUtf8());
	}
	_bettergramChanged.fire({});
}

QString Settings::aiComposeApiModel() const {
	if (const auto data = readPrefGeneric("ai-compose-api-model")) {
		return QString::fromUtf8(*data);
	}
	return {};
}
void Settings::setBettergramHubUrl(const QString &value) {
	if (value.isEmpty()) { clearPref("bettergram-hub-url"); }
	else { writePrefGeneric("bettergram-hub-url", value.toUtf8()); }
	_bettergramChanged.fire({});
}
QString Settings::bettergramHubUrl() const {
	if (const auto data = readPrefGeneric("bettergram-hub-url")) {
		return QString::fromUtf8(*data);
	}
	return {};
}
void Settings::setBgAvatarUrl(const QString &value) {
	if (value.isEmpty()) { clearPref("bg-avatar-url"); }
	else { writePrefGeneric("bg-avatar-url", value.toUtf8()); }
	_bettergramChanged.fire({});
}
QString Settings::bgAvatarUrl() const {
	if (const auto data = readPrefGeneric("bg-avatar-url")) {
		return QString::fromUtf8(*data);
	}
	return {};
}
void Settings::setBgNameColor(const QString &value) {
	if (value.isEmpty()) { clearPref("bg-name-color"); }
	else { writePrefGeneric("bg-name-color", value.toUtf8()); }
	_bettergramChanged.fire({});
}
QString Settings::bgNameColor() const {
	if (const auto data = readPrefGeneric("bg-name-color")) {
		return QString::fromUtf8(*data);
	}
	return {};
}
void Settings::setBgHubAdminToken(const QString &value) {
	if (value.isEmpty()) { clearPref("bg-hub-admin-token"); }
	else { writePrefGeneric("bg-hub-admin-token", value.toUtf8()); }
	_bettergramChanged.fire({});
}
QString Settings::bgHubAdminToken() const {
	if (const auto data = readPrefGeneric("bg-hub-admin-token")) {
		return QString::fromUtf8(*data);
	}
	return {};
}

void Settings::setBgGhostMessages(bool value) {
	if (!value) { clearPref("bg-ghost-messages"); }
	else { writePrefGeneric("bg-ghost-messages", QByteArray("1")); }
	_bettergramChanged.fire({});
}
bool Settings::bgGhostMessages() const {
	return readPrefGeneric("bg-ghost-messages").has_value();
}
void Settings::setBgAlwaysShowTimestamps(bool value) {
	if (!value) { clearPref("bg-always-timestamps"); }
	else { writePrefGeneric("bg-always-timestamps", QByteArray("1")); }
	_bettergramChanged.fire({});
}
bool Settings::bgAlwaysShowTimestamps() const {
	return readPrefGeneric("bg-always-timestamps").has_value();
}
void Settings::setBgCharCounter(bool value) {
	if (!value) { clearPref("bg-char-counter"); }
	else { writePrefGeneric("bg-char-counter", QByteArray("1")); }
	_bettergramChanged.fire({});
}
bool Settings::bgCharCounter() const {
	return readPrefGeneric("bg-char-counter").has_value();
}
void Settings::setBgLinkDeTracker(bool value) {
	if (!value) { clearPref("bg-link-detracker"); }
	else { writePrefGeneric("bg-link-detracker", QByteArray("1")); }
	_bettergramChanged.fire({});
}
bool Settings::bgLinkDeTracker() const {
	return readPrefGeneric("bg-link-detracker").has_value();
}
QVector<uint64> Settings::bgTypingSuppressedPeers() const {
	QVector<uint64> result;
	if (const auto data = readPrefGeneric("bg-typing-suppress")) {
		const auto str = QString::fromUtf8(*data);
		for (const auto &part : str.split(QChar(','), Qt::SkipEmptyParts)) {
			bool ok = false;
...
void Settings::addBgTypingSuppressedPeer(uint64 peerId) {
	auto list = bgTypingSuppressedPeers();
	if (!list.contains(peerId)) {
		list.push_back(peerId);
	}
	QStringList parts;
	for (const auto id : list) parts.push_back(QString::number(id));
	writePrefGeneric("bg-typing-suppress", parts.join(u","_q).toUtf8());
	_bettergramChanged.fire({});
}
void Settings::removeBgTypingSuppressedPeer(uint64 peerId) {
	auto list = bgTypingSuppressedPeers();
	list.removeAll(peerId);
	if (list.isEmpty()) {
		clearPref("bg-typing-suppress");
		_bettergramChanged.fire({});
		return;
	}
	QStringList parts;
...
bool Settings::isBgTypingSuppressed(uint64 peerId) const {
	return bgTypingSuppressedPeers().contains(peerId);
}
void Settings::setBgPeerNote(uint64 peerId, const QString &note) {
	const auto key = u"peer-note-%1"_q.arg(peerId).toStdString();
	if (note.isEmpty()) { clearPref(key); }
	else { writePrefGeneric(key, note.toUtf8()); }
}
QString Settings::bgPeerNote(uint64 peerId) const {
	const auto key = u"peer-note-%1"_q.arg(peerId).toStdString();
	if (const auto data = readPrefGeneric(key)) {
		return QString::fromUtf8(*data);
	}
...
void Settings::setBgPeerSound(uint64 peerId, const QString &path) {
	const auto key = u"peer-sound-%1"_q.arg(peerId).toStdString();
	if (path.isEmpty()) { clearPref(key); }
	else { writePrefGeneric(key, path.toUtf8()); }
}
QString Settings::bgPeerSound(uint64 peerId) const {
	const auto key = u"peer-sound-%1"_q.arg(peerId).toStdString();
	if (const auto data = readPrefGeneric(key)) {
		return QString::fromUtf8(*data);
	}
...
void Settings::setBgThemeName(const QString &name) {
	if (name.isEmpty()) { clearPref("bg-theme-name"); }
	else { writePrefGeneric("bg-theme-name", name.toUtf8()); }
	_bettergramChanged.fire({});
}
QString Settings::bgThemeName() const {
	if (const auto data = readPrefGeneric("bg-theme-name")) {
		return QString::fromUtf8(*data);
	}
	return {};
}
void Settings::setBgHubThemeSync(bool value) {
	if (!value) { clearPref("bg-hub-theme-sync"); }
	else { writePrefGeneric("bg-hub-theme-sync", QByteArray("1")); }
	_bettergramChanged.fire({});
}
bool Settings::bgHubThemeSync() const {
	return readPrefGeneric("bg-hub-theme-sync").has_value();
}
void Settings::setBgMiniPlayerEnabled(bool value) {
	// Inverted: store the disabled flag so default is enabled.
	if (value) { clearPref("bg-mini-player-off"); }
	else { writePrefGeneric("bg-mini-player-off", QByteArray("1")); }
	_bettergramChanged.fire({});
}
bool Settings::bgMiniPlayerEnabled() const {
	return !readPrefGeneric("bg-mini-player-off").has_value();
}

void Settings::setHideStoriesBar(bool value) {
	if (!value) { clearPref("hide-stories-bar"); }
	else { writePrefGeneric("hide-stories-bar", QByteArray("1")); }
	_bettergramChanged.fire({});
}
bool Settings::hideStoriesBar() const {
	return readPrefGeneric("hide-stories-bar").has_value();
}

void Settings::setHideStoryBadges(bool value) {
	if (!value) { clearPref("hide-story-badges"); }
	else { writePrefGeneric("hide-story-badges", QByteArray("1")); }
	_bettergramChanged.fire({});
}
bool Settings::hideStoryBadges() const {
	return readPrefGeneric("hide-story-badges").has_value();
}

void Settings::setHideStoryNotifications(bool value) {
	if (!value) { clearPref("hide-story-notifications"); }
	else { writePrefGeneric("hide-story-notifications", QByteArray("1")); }
	_bettergramChanged.fire({});
}
bool Settings::hideStoryNotifications() const {
	return readPrefGeneric("hide-story-notifications").has_value();
}

void Settings::setHidePremiumBadges(bool value) {
	if (!value) { clearPref("hide-premium-badges"); }
	else { writePrefGeneric("hide-premium-badges", QByteArray("1")); }
	_bettergramChanged.fire({});
}
bool Settings::hidePremiumBadges() const {
	return readPrefGeneric("hide-premium-badges").has_value();
}

void Settings::setHidePremiumUpsells(bool value) {
	if (!value) { clearPref("hide-premium-upsells"); }
	else { writePrefGeneric("hide-premium-upsells", QByteArray("1")); }
	_bettergramChanged.fire({});
}
bool Settings::hidePremiumUpsells() const {
	return readPrefGeneric("hide-premium-upsells").has_value();
}

void Settings::setHidePremiumMenuItems(bool value) {
	if (!value) { clearPref("hide-premium-menu-items"); }
	else { writePrefGeneric("hide-premium-menu-items", QByteArray("1")); }
	_bettergramChanged.fire({});
}
bool Settings::hidePremiumMenuItems() const {
	return readPrefGeneric("hide-premium-menu-items").has_value();
}

void Settings::setHideUpgradeButtons(bool value) {
	if (!value) { clearPref("hide-upgrade-buttons"); }
	else { writePrefGeneric("hide-upgrade-buttons", QByteArray("1")); }
	_bettergramChanged.fire({});
}
bool Settings::hideUpgradeButtons() const {
	return readPrefGeneric("hide-upgrade-buttons").has_value();
}
void Settings::setHideSponsoredMessages(bool value) {
	if (!value) { clearPref("hide-sponsored-messages"); }
	else { writePrefGeneric("hide-sponsored-messages", QByteArray("1")); }
	_bettergramChanged.fire({});
}
bool Settings::hideSponsoredMessages() const {
	return readPrefGeneric("hide-sponsored-messages").has_value();
}
