...
void Application::startSettingsAndBackground() {
	...
	checkSystemDarkMode();
	BetterGram::RestoreTheme();
	// Hub theme sync: fetched after local theme so it takes priority.
	if (settings().bgHubThemeSync()) {
		BetterGram::Hub::instance().fetchHubTheme(
			[](std::optional<BetterGram::BgPalette> palette) {
				if (palette) {
					// Apply visually without persisting — local name is
					// restored on next restart if hub theme disappears.
					Window::Theme::ApplyEditedPalette(
						BetterGram::DerivePalette(*palette));
				}
			});
	}
