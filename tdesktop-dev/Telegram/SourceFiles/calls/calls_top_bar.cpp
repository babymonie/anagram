...
, _bgMiniButton(groupCall && Core::App().settings().bgMiniPlayerEnabled()
	? object_ptr<Ui::IconButton>(this, st::callBarMini)
	: object_ptr<Ui::IconButton>(nullptr))
...
void TopBar::initControls() {
	...
	if (_bgMiniButton) {
		_bgMiniButton->setClickedCallback([this] {
			if (_bgMiniPlayer) {
				_bgMiniPlayer.reset();
				return;
			}
			...
			if (!group) return;
			_bgMiniPlayer = std::make_unique<BetterGram::BgCallMini>(
				group,
				[this] { Core::App().calls().showInfoPanel(_groupCall.get()); });
		});
	}
...
void TopBar::updateControlsGeometry() {
	...
	const auto miniW = _bgMiniButton ? st::callBarMini.width : 0;
	if (_bgMiniButton) {
		_bgMiniButton->setGeometryToRight(_hangup->width(), 0, miniW, height());
	}
	_info->setGeometryToLeft(
		_mute->width(),
		0,
		width() - _mute->width() - _hangup->width() - miniW,
		height());
