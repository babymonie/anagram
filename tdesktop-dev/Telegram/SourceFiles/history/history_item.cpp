			...
			if (!_media) {
				for (const auto &entity : textWithEntities.entities) {
					if (entity.type() == EntityType::Url
						...
						const auto url = (entity.type() == EntityType::CustomUrl)
							? entity.data()
							: textWithEntities.text.mid(
								entity.offset(),
								entity.length());
						if (const auto ef = BetterGram::Hub::detectExternalFile(url)) {
							_media = std::make_unique<Data::MediaBgExternalFile>(
								this,
								*ef);
							break;
						}
...
void HistoryItem::setBgGhostDeleted() {
	_bgGhostDeleted = true;
}
