...
void AddHubReactionAction(
	not_null<Ui::PopupMenu*> menu,
	const ContextMenuRequest &request,
	not_null<ListWidget*> list);
void AddHubRelayAction(
	not_null<Ui::PopupMenu*> menu,
	const ContextMenuRequest &request,
	not_null<ListWidget*> list);
...
void AddMessageActions(
	...
	AddHubReactionAction(menu, request, list);
	AddHubRelayAction(menu, request, list);
}
...
void AddHubReactionAction(
	...
	const auto controller = list->controller();
	const auto peerId = int64(item->history()->peer->id.value);
	const auto msgId = int32(item->id.bare);
	const auto myTgId = controller->session().userId().bare;

	menu->addAction(tr::lng_bettergram_hub_react(tr::now), [=] {
		controller->show(Box([=](not_null<Ui::GenericBox*> box) {
			box->setTitle(tr::lng_bettergram_hub_react());
			const auto field = box->addRow(object_ptr<Ui::InputField>(
				box,
				st::defaultInputField,
				tr::lng_bettergram_hub_react_placeholder(),
				QString()));
			const auto send = [=] {
				const auto emoji = field->getLastText().trimmed();
				if (emoji.isEmpty()) {
					return;
				}
				BetterGram::Hub::instance().addReaction(
					peerId,
					msgId,
					myTgId,
					emoji);
				controller->showToast(tr::lng_bettergram_hub_reacted(tr::now));
				box->closeBox();
			};
			field->submits() | rpl::on_next(send, field->lifetime());
			box->addButton(tr::lng_bettergram_hub_react(), send);
			box->addButton(tr::lng_cancel(), [=] { box->closeBox(); });
		}));
	}, &st::menuIconReactions);

	menu->addAction(tr::lng_bettergram_hub_view_reactions(tr::now), [=] {
		BetterGram::Hub::instance().fetchReactions(
			peerId,
			msgId,
			[=](QVector<BetterGram::HubReaction> reactions) {
				controller->show(Box([=](not_null<Ui::GenericBox*> box) {
					box->setTitle(tr::lng_bettergram_hub_view_reactions());
					if (reactions.isEmpty()) {
						box->addRow(object_ptr<Ui::FlatLabel>(
							box,
							tr::lng_bettergram_hub_reactions_empty(),
							st::boxDividerLabel));
					}
					auto mine = std::optional<QString>();
					...
					for (const auto &[emoji, count] : counts) {
						box->addRow(object_ptr<Ui::FlatLabel>(
							box,
							rpl::single(u"%1  x%2"_q.arg(emoji).arg(count)),
							st::boxDividerLabel));
					}
					if (mine) {
						box->addButton(
							tr::lng_bettergram_hub_remove_reaction(),
							[=] {
								BetterGram::Hub::instance().removeReaction(
									peerId,
									msgId,
									myTgId);
								box->closeBox();
...
void AddHubRelayAction(
		...
		not_null<ListWidget*> list) {
	const auto item = request.item;
	if (!item || !BetterGram::Hub::instance().isConfigured()) {
		return;
	}
	const auto media = item->media();
	const auto external = media
		? dynamic_cast<Data::MediaBgExternalFile*>(media)
		: nullptr;
	if (!external) {
		return;
	}
	const auto controller = list->controller();
	const auto file = external->externalFile();
	menu->addAction(tr::lng_bettergram_hub_get_relay_link(tr::now), [=] {
		BetterGram::Hub::instance().registerRelay(
			file.url,
			file.filename,
			file.size,
			file.mime,
			[=](std::optional<BetterGram::HubRelay> relay) {
				if (!relay || relay->shortUrl.isEmpty()) {
					controller->showToast(
						tr::lng_bettergram_hub_relay_failed(tr::now));
					return;
				}
				QGuiApplication::clipboard()->setText(relay->shortUrl);
				controller->showToast(
					tr::lng_bettergram_hub_relay_copied(tr::now));
			});
	}, &st::menuIconCopy);
}
