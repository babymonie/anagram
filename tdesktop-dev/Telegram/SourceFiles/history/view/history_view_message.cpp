...
void Message::draw(Painter &p, const PaintContext &context) const {
	...
	// BG Ghost Messages: dim the bubble for remotely-deleted ghost items.
	const auto isGhost = item->isBgGhostDeleted();
	if (isGhost) {
		p.save();
		p.setOpacity(0.45);
	}
	...
	// BG Ghost Messages: keep the original content visible, only dimmed.
	if (isGhost) {
		p.restore();
		return;
	}
...
void Message::paintFromName(
	...
	// Hub role badge — drawn inline after the name when the sender is a BG user.
	{
		const auto fromPeer = item->displayFrom();
		const auto senderId = fromPeer ? int64(fromPeer->id.value) : int64(0);
		auto &hub = BetterGram::Hub::instance();
		if (senderId && hub.isKnownBgUser(senderId)) {
			// Kick off a profile fetch the first time we encounter this sender
			// on this view.  The callback captures only the HistoryItem* (which
			// outlives any view) and re-requests a repaint so we re-enter here
			// with the profile now in cache.
			if (!_bgHubBadgeFetched) {
				_bgHubBadgeFetched = 1;
				const auto itemPtr = item; // not_null<HistoryItem*>
				hub.fetchProfile(senderId, [itemPtr](auto) {
					itemPtr->history()->owner().requestItemRepaint(itemPtr);
				});
			}
			// Presence dot — small green dot before the badge if the hub's
			// last heartbeat for this sender is still recent.
			if (const auto lastSeen = hub.cachedLastSeen(senderId)) {
				if (BetterGram::Hub::IsRecentlySeen(*lastSeen) && availableWidth > 0) {
					const auto dotDiameter = st::msgServiceFont->height / 3;
					const auto dotRect = QRect(
						availableLeft,
						trect.top() + (st::msgNameFont->height - dotDiameter) / 2,
						dotDiameter,
						dotDiameter);
					if (availableWidth >= dotDiameter + st::msgServiceFont->spacew) {
						auto hq = PainterHighQualityEnabler(p);
						p.setPen(Qt::NoPen);
						p.setBrush(QColor(76, 217, 100)); // iOS-style online green
						p.drawEllipse(dotRect);
						const auto skip = dotDiameter + st::msgServiceFont->spacew;
						availableLeft += skip;
						availableWidth -= skip;
					}
				}
			}
			// Synchronous cache read — returns badge text + color if available.
			if (const auto badge = hub.cachedBadge(senderId)) {
				const auto &badgeText = badge->first;
				const auto &badgeColorStr = badge->second;
				if (!badgeText.isEmpty() && availableWidth > 0) {
					const auto &f = st::msgServiceFont;
					const auto textW = f->width(badgeText);
					const auto padH = 1;
					const auto padW = 4;
					const auto pillH = f->height + 2 * padH;
					const auto pillW = textW + 2 * padW;
					if (availableWidth >= pillW + f->spacew) {
						availableLeft += f->spacew;
						availableWidth -= f->spacew;
						const QRect pill(
							availableLeft,
							trect.top() + (st::msgNameFont->height - pillH) / 2,
							pillW,
							pillH);
						QColor pillColor(badgeColorStr);
						if (!pillColor.isValid()) {
							pillColor = QColor(91, 127, 241);
						}
						auto bgCol = pillColor;
						bgCol.setAlphaF(0.18);
						{
							auto hq = PainterHighQualityEnabler(p);
							p.setPen(Qt::NoPen);
							p.setBrush(bgCol);
						...
						p.drawText(
							pill.left() + padW,
							pill.top() + padH + f->ascent,
							badgeText);
						availableLeft += pillW;
						availableWidth -= pillW;
					}
				}
			}
		}
...
bool Message::needInfoDisplay() const {
	// BG Always-Show Timestamps: force info visible for all message types.
	if (Core::App().settings().bgAlwaysShowTimestamps()) {
		return true;
	}
	const auto media = this->media();
	const auto mediaDisplayed = media ? media->isDisplayed() : false;
