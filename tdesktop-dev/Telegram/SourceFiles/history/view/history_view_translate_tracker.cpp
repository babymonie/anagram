...
void TranslateTracker::setup() {
	...
	// A configured custom translate server (LibreTranslate/Lingva/etc.)
	// unlocks translation the same way Premium or the channel's own
	// auto-translation flag does, since it doesn't depend on Telegram's
	// own (Premium-gated) translation backend.
	auto hasCustomProviderValue = rpl::single(
		rpl::empty_value()
	) | rpl::then(
		Core::App().settings().bettergramChanges()
	) | rpl::map([] {
		return !Core::App().settings().translateUrlTemplate().isEmpty();
	}) | rpl::distinct_until_changed();
	...
	_trackingLanguage = rpl::combine(
		...
		std::move(hasCustomProviderValue),
		_1 && (_2 || _3 || _4));
	...
	// Recreate the provider whenever the custom translate server URL
	// changes, so an already-open chat picks up the new server instead
	// of staying pinned to whatever provider was resolved at construction.
	Core::App().settings().bettergramChanges(
	) | rpl::on_next([=] {
		cancelToRequest();
		cancelSentRequest();
...
void TranslateTracker::requestSome() {
	...
	_provider->requestBatch(
		...
		[=](int index, Ui::TranslateProviderResult result) {
			...
			if (!result.text) {
				DEBUG_LOG(("Translate Warning: Auto-translate request "
					"failed for message %1_%2, error %3."
					).arg(id.peer.value
					).arg(id.msg.bare
