...
void ComposeControls::checkCharsLimitation() {
	// Determine the over-limit state first (existing logic).
	int remove = 0;
	if (_history && isEditingMessage()) {
		...
		if (item) {
			...
			remove = Ui::ComputeFieldCharacterCount(_field) - maxTextSize;
		}
	}

	if (remove > 0) {
		// Over limit — show the limit label, hide the BG counter.
		if (!_charsLimitation) {
		...
		_bgCharCounter = nullptr;
	} else {
		_charsLimitation = nullptr;
		// BG Character Counter: always-visible typed-char count when enabled.
		if (Core::App().settings().bgCharCounter()) {
			const auto count = Ui::ComputeFieldCharacterCount(_field);
			if (!_bgCharCounter) {
				using namespace Controls;
				_bgCharCounter = base::make_unique_q<CharactersLimitLabel>(
					_wrap.get(),
					_send.get(),
					style::al_bottom);
				_bgCharCounter->show();
			}
			// setLeft(−count) renders the raw number in grey without a "−" prefix.
			_bgCharCounter->setLeft(-count);
		} else {
			_bgCharCounter = nullptr;
		}
	}
