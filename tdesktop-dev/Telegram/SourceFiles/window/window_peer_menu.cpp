...
void Filler::addDownloadMedia() {
	...
	const auto doDownload = [=](int limit) {
		const auto session = &peer->session();
		const auto weak = base::make_weak(controller);

		{
			const auto sharedCount = std::make_shared<int>(0);
				...
				session->api().request(MTPmessages_Search(
					...
					for (const auto &msg : messages) {
						msg.match([&](const MTPDmessage &d) {
							...
							const auto media = d.vmedia()->match(
								[&](const MTPDmessageMediaDocument &m)
									-> std::optional<not_null<DocumentData*>> {
									const auto &doc = m.vdocument();
									return doc
										? session->data().processDocument(*doc)
										: std::optional<not_null<DocumentData*>>();
								},
								[](const auto &)
									-> std::optional<not_null<DocumentData*>> {
					...
					if (const auto strong = weak.get()) {
						strong->showToast(
							(*sharedCount > 0)
								? tr::lng_bettergram_download_starting(tr::now)
								: tr::lng_bettergram_download_none(tr::now));
					}
				}).send();
		}
	};
	...
	_addAction(
		tr::lng_bettergram_download_last_100(tr::now),
		[=] { doDownload(100); },
		&st::menuIconDownload);
	_addAction(
		tr::lng_bettergram_download_last_500(tr::now),
		[=] { doDownload(500); },
		&st::menuIconDownload);
	_addAction(
		tr::lng_bettergram_download_all_media(tr::now),
		[=] { doDownload(2000); },
		&st::menuIconDownload);
}

void Filler::addContactNote() {
	if (!_peer || _peer->isSelf() || _peer->isChat() || _peer->isChannel()) {
		return;
	}
	const auto peer = _peer;
	const auto controller = _controller;
	const auto peerId = peer->id.value;
	const auto existingNote = Core::App().settings().bgPeerNote(peerId);
	_addAction(
		existingNote.isEmpty()
			? tr::lng_bettergram_note_add(tr::now)
			: tr::lng_bettergram_note_edit(tr::now),
		[=] {
			controller->show(Box([=](not_null<Ui::GenericBox*> box) {
				box->setTitle(tr::lng_bettergram_note_title());
				const auto field = box->addRow(
					object_ptr<Ui::InputField>(
						box,
						st::defaultInputField,
						tr::lng_bettergram_note_placeholder(),
						Core::App().settings().bgPeerNote(peerId)));
				const auto save = [=] {
					Core::App().settings().setBgPeerNote(
						peerId,
		...
		existingNote.isEmpty() ? &st::menuIconEdit : &st::menuIconEdit);
}
...
void Filler::addSendGift() {
	...
	if (Core::App().settings().hidePremiumMenuItems()) {
		return;
	}
	const auto peer = _peer;
	const auto navigation = _controller;
